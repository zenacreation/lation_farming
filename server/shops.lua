...
--- @param itemId number
--- @param input number
RegisterNetEvent('lation_farming:completepurchase', function(itemId, input)
    ...
    if item.level then
        ...
        if level < item.level then
            TriggerClientEvent('lation_farming:notify', source, locale('notify.not-experienced'), 'error')
            return
        end
    end
    ...
    if not balance or balance < total then
        TriggerClientEvent('lation_farming:notify', source, locale('notify.no-money'), 'error')
        return
    end
...
--- @param itemId number
--- @param input number
RegisterNetEvent('lation_farming:completesale', function(itemId, input)
    if not source or not itemId or not input then return end
    ...
    if not hasItem then
        TriggerClientEvent('lation_farming:notify', source, locale('notify.no-item'), 'error')
        return
    end
    ...
    farming:AddPlayerData(source, 'earned', total)
