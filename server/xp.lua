...
local sql = LoadResourceFile(GetCurrentResourceName(), 'install/lation_farming.sql')
...
-- Returns player data from lation_farming table
---@param source number
...
local function GetPlayerData(source, type)
    ...
    if not data then
        local query = [[ SELECT * FROM `lation_farming` WHERE `identifier` = ? ]]
...
lib.callback.register('lation_farming:gettopplayers', function()
    return GetTopPlayers()
end)

-- Callback used to return players data from lation_farming
--- @param source number
--- @param type string|nil
lib.callback.register('lation_farming:getplayerdata', function(source, type)
...
-- Register export(s)
exports('GetPlayerData', GetPlayerData) -- returns player data from lation_farming table (params: source, type) (type param optional)
exports('AddPlayerData', AddPlayerData) -- edit player data in lation_farming table (params: source, type, amount)