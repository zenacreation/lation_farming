...
local function buildMenu()
    for ingotId, data in ipairs(shared.smelting.ingots) do
        ...
        menu[#menu + 1] = {
            ...
            icon = data.icon,
            event = 'lation_farming:smelting:selectquantity',
            args = ingotId
        }
    end
...
local function startSmelting(ingotId, count)
    ...
    for i = 1, count do
        ...
        smelted += 1
        TriggerServerEvent('lation_farming:completesmelt', ingotId)
    end
...
-- Select quantity
--- @param ingotId number
AddEventHandler('lation_farming:smelting:selectquantity', function(ingotId)
