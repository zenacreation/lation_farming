...
--- @param item string
--- @return boolean
local function isShovel(item)
    for _, data in pairs(shared.shovels) do
...
local function openShop()
    ...
    local player = farming:GetPlayerData()
    ...
    if client.leaderboard then
        ...
        local players = lib.callback.await('lation_farming:gettopplayers', false)
...
local function createShop()
    ...
    for id, data in ipairs(shared.shops.mine.items) do
        ...
        local isPick = isShovel(data.item)
        themines.shop[#themines.shop + 1] = {
            title = isPick and locale('shop-menu.shovel-title', itemData.label, id) or itemData.label,
            ---@diagnostic disable-next-line: undefined-field
            ...
            image = isPick and 'nui://lation_farming/install/images/'..data.item..'.png' or nil,
            event = 'lation_farming:shops:selectquantity',
            args = id
        }
    end
    lib.registerContext({
    ...
    for id, data in ipairs(shared.shops.pawn.items) do
        ...
        themines.pawn[#themines.pawn + 1] = {
            ...
            icon = data.icon or 'fas fa-triangle-exclamation',
            iconColor = data.iconColor or '',
            event = 'lation_farming:pawn:selectquantity',
            args = id
        }
    end
    lib.registerContext({
...
-- Select quantity
--- @param id number
AddEventHandler('lation_farming:shops:selectquantity', function(id)
    ...
    TriggerServerEvent('lation_farming:completepurchase', id, input[1])
end)

-- Select quantity
--- @param id number
AddEventHandler('lation_farming:pawn:selectquantity', function(id)
    if not id then return end
    ...
    if not input or not input[1] then return end
    TriggerServerEvent('lation_farming:completesale', id, input[1])
end)
