...
function ShowNotification(message, type)
    if shared.setup.notify == 'ox_lib' then
        lib.notify({ description = message, type = type, position = 'center-right', icon = 'fas fa-fire' })
    elseif shared.setup.notify == 'esx' then
        ESX.ShowNotification(message)
    elseif shared.setup.notify == 'qb' then
        QBCore.Functions.Notify(message, type)
    elseif shared.setup.notify == 'okok' then
        exports['okokNotify']:Alert('farming', message, 5000, type, false)
    elseif shared.setup.notify == 'sd-notify' then
        exports['sd-notify']:Notify('farming', message, type)
    elseif shared.setup.notify == 'wasabi_notify' then
        exports.wasabi_notify:notify('farming', message, 5000, type, false, 'fas fa-fire')
...
--- @param message string
--- @param type string
RegisterNetEvent('lation_farming:notify', function(message, type)
